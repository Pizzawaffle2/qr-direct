      ...
      try {
        ...
        setUsage({
          qrCodes: {
            ...
            percentage: plan.limits.qrCodes === -1 ? 0 : 
              (data.qrCodesCreated / plan.limits.qrCodes) * 100,
          },
          templates: {
            current: data.templatesCreated,
            limit: plan.limits.templates,
            percentage: plan.limits.templates === -1 ? 0 :
          ...
          storage: {
            ...
            percentage: plan.limits.storage === -1 ? 0 :
              (data.storageUsed / plan.limits.storage) * 100,
          },
          scans: {
            current: data.scans,
            limit: plan.limits.scanLimit,
            percentage: plan.limits.scanLimit === -1 ? 0 :
              (data.scans / plan.limits.scanLimit) * 100,
          },
        })
      } catch (error) {
