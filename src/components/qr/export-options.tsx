...
export function ExportOptions({ settings, className = '', disabled = false }: ExportOptionsProps) {
...
function ExportMenuItem({ format, isExporting, onClick }: ExportMenuItemProps) {
  return (
    <DropdownMenuItem disabled={isExporting} onClick={onClick} className="flex items-center">
      {isExporting ? (
        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
      ) : (
        <Download className="mr-2 h-4 w-4" />
