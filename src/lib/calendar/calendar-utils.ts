...
import type { CalendarSettings, CalendarEvent, LunarPhase } from '@/types/calendar-types';
...
export const getEventColor = (event: CalendarEvent, settings: CalendarSettings): string => {
  ...
  const defaultColors: Record<'holiday' | 'event' | 'reminder', string> = {
    holiday: settings.theme.accentColor,
    event: settings.theme.headerColor,
    reminder: settings.theme.textColor,
  };

  return defaultColors[event.type as keyof typeof defaultColors] || settings.theme.textColor;
};
